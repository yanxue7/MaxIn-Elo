...
def smooth(Y,k):
    ...
    return y_smooth
def picture(hit,NDCG,label,linestyle='-',linewidth=1.5):
    k=10
    hit=hit[:,:k]
    NDCG=NDCG[:,:k]
    hit=smooth(hit,k)
...
def process_data(game,model,mode):
    ...
    for alpha in parameters['alpha']:
        ...
        Reg=np.array(Reg)
        Hit=np.array(Hit)
        picture(Hit,NDCG,label=r"$\gamma=$"+str(alpha))
    ...
    plt.show()
def draw_baselines():
    models = ['random', 'RGUCB', 'DBGD', 'MaxInP']
    for model in models:
        path = "AAAIELO/Avg/{}/{}/".format(game,model)
        if para_seed=='max':

            paras = json.load(open('AAAIELO/Avg/{}/{}/max_best_para.json'.format(game, model), 'r'))
        else:
            paras = json.load(open('AAAIELO/Avg/{}/{}/best_para.json'.format(game, model), 'r'))
        NDCG = []
        Reg = []
        Hit = []
        for para in paras:
            ...
            NDCG.append(ndcg)
            Hit.append(hit)
            para_list=para.split('_')
            alpha=para_list[1]
        NDCG = np.array(NDCG)
        Reg = np.array(Reg)
        Hit = np.array(Hit)
        if model=='MaxInP':
            model=r'$\gamma={}$'.format(alpha)
        if model=='random':
            model='Random'
        if model == 'RGUCB':
            model = 'RG-UCB'
        picture(Hit, NDCG, label=model,linestyle='-')
if __name__=='__main__':
    alpha_set={}
    alpha_set['Elo game']=[0.8]
    alpha_set['Transitive game'] = [0.4]
    alpha_set['Triangular game'] = [1]
    alpha_set['Elo game + noise=0.1'] =[0.8]
    alpha_set['Elo game + noise=0.05'] = [1]
    alpha_set['Elo game + noise=0.01'] = [1]
