...
def get_rates(path,batch):
    if mode=='Avg':
        paras=json.load(open(path+'best_para.json','r'))
    else:
        paras = json.load(open(path + 'max_best_para_batch_{}_{}.json'.format(batch,melo), 'r'))
    rates=[]
    Reg=[]
    print(paras)
    for i in range(rep):
...
if __name__=='__main__':
    ...
    rep=5
    if melo==0:
        data_path='AAAIELO_Batch/'+'Avg/'
    else:
        data_path = 'AAAIMELO_Batch/' + 'Avg/'
    for game_name in games:
        ...
        name ='MaxInELO'
        batchs=[0.0, 0.1, 0.3, 0.5, 0.7, 0.9, 1.0, 2.0, 4.0, 8.0]
        for batch in batchs:
            rates,Reg=get_rates(data_path+game_name+'/'+name+'/',batch)

            if batch==0.0:
                picture(Reg,label=1)
            else:
                picture(Reg, label='{}*n'.format(batch))
        plt.subplot(1, 2, 1)
        ...
        plt.savefig(path+'{}_{}_batch.pdf'.format(game_name,mode))
        plt.show()
